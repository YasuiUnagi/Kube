...
namespace kube::video {
    ...
    struct alignas(T) Vec2D final {
        ...
        constexpr Vec2D(T x, T y) : array {x, y} {
        }
    ...
    template < typename T >
    inline auto hypot(const Vec2D<T> &v) noexcept
        -> T = delete;
    
    // Hypothesis of 2d f64 vector.
    template <>
    inline auto hypot(const Vec2D<f64> &v) noexcept
        -> f64 { return {math::hypot(v.x(), v.y())}; }

    // Hypothesis of 2d f32 vector.
    template <>
    inline auto hypot(const Vec2D<f32> &v) noexcept
